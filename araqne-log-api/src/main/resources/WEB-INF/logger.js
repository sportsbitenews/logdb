Logger = function() {
	...
	this.onstart = function(pid, args) {
		...
		function getLoggers(callback) {
			channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.getLoggers', {},
				function(resp) {
		...
		function removeLoggers() {
			...
			Ext.Msg.confirm('Remove Loggers', 'It will remove logger "' + sel.toString() + '"<br/>Are you sure?', function(btn) {
				if(btn == 'yes') {
					channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.removeLoggers',
						params,
						function(resp) {
		...
		function startLogger(name, interval) {
			...
			channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.startLogger', 
				{
					'logger': name,
					'interval': interval
				},
		...
		function stopLogger(name) {
			...
			channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.stopLogger', 
				{
					'logger': name,
		...
		function getLoggerFactories(callback) {
			channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.getLoggerFactories',
				{
		...
		function getParsers(callback) {
			channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.getParsers', {},
				function(resp) {
		...
		function getNormalizers(callback) {
			channel.send(1, 'org.araqne.log.api.msgbus.LoggerPlugin.getNormalizers', {},
				function(resp) {
