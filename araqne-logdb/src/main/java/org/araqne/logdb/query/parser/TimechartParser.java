...
public class TimechartParser extends AbstractQueryCommandParser {
	...
	public TimechartParser() {
		setDescriptions("Evaluate aggregatation function per time span. If you specify by-clause, fields are generated by group.",
				"지정된 시간 단위마다 집계 함수의 결과를 계산합니다. by 절을 이용하여 그룹 필드를 지정하는 경우, 그룹 필드 값으로 필드가 생성되면서 필드별 통계 값을 계산합니다.");

		setOptions("span", true,
	...
	public QueryCommand parse(QueryContext context, String commandString) {
		...
		if (fields.size() == 0)
			// throw new QueryParseException("need-aggregation-field",
			// COMMAND.length());
