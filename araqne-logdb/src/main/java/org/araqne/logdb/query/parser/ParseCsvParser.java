...
public class ParseCsvParser extends AbstractQueryCommandParser {
	...
	public ParseCsvParser() {
		...
		setOptions("field", "Target field name", "파싱할 대상 필드 이름");
		setOptions("overlay", "Return also original field (t or f)", "CSV로 파싱된 결과 외에 원본 필드 값도 포함할지 설정합니다. t 혹은 f");
		setOptions("tab", "Use tab to delimiter. (t or f)", "CSV로 파싱된 결과 외에 원본 필드 값도 포함할지 설정합니다. t 혹은 f");
