...
import org.araqne.logstorage.file.BufferedRandomAccessFileReader;
...
public class InvertedIndexReader {
	...
	public class Cursor implements InvertedIndexCursor {
		...
		private Long loadSegment(long segmentIndex) throws IOException {
			...
			logger.debug("araqne logstorage: index data end offset [{}]", dataEndOffset);
			...
			logger.debug("araqne logstorage: index term block length [{}]", termBlockLength);

			long postingBlockLength = nextLong();
			logger.debug("araqne logstorage: posting block length [{}]", postingBlockLength);

			// find target term while iterate
			long beginOfTermBlock = dataPos - termBlockLength;
			logger.debug("araqne logstorage: begin [{}], data pos [{}]", beginOfTermBlock, dataPos);
			...
			while (dataPos > beginOfTermBlock) {
				...
				if (logger.isDebugEnabled())
					logger.debug("araqne logstorage: term {}, count {}, offset {}", new Object[] { t, termCount, offset });
			}
			...
			logger.debug("araqne logstorage: data [{}~{}] term [{}] posting block length [{}] posting offset [{}]", new Object[] {
