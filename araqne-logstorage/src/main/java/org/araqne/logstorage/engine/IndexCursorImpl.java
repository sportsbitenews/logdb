...
import org.araqne.logstorage.index.InvertedIndexCursor;
import org.araqne.logstorage.index.InvertedIndexItem;
import org.araqne.logstorage.index.InvertedIndexReader;
	...
	public IndexCursorImpl(int indexId, int tableId, String tableName, List<Date> days, String term, File indexBaseDir,
		...
		if (days.size() > 0) {
			try {
				...
				logger.error("araqne logstorage: cannot load index file, skipping", t);
				tryLoadUntilSuccess();
			}
		}
	}
	...
	public boolean hasNext() {
		...
		if (currentCursor.hasNext()) {
			try {
				...
				logger.error("araqne logstorage: cannot fetch next index item from cursor", e);
			}
		}
		...
		try {
			...
			logger.error("araqne logstorage: cannot load next indexed day from cursor", e);
		}
