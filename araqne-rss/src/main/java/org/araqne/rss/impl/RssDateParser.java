...
public class RssDateParser {
	public static Date parse(String date) {
		try {
			...
			if (Pattern.matches(datePattern1, date) | Pattern.matches(datePattern2, date))
			...
			} else if (Pattern.matches(datePattern5, date) || Pattern.matches(datePattern6, date)) {
				Calendar cal = Calendar.getInstance();
				String time = String.format("%02d:%02d:%02d", cal.get(Calendar.HOUR_OF_DAY), cal.get(Calendar.MINUTE),
			...
			java.text.SimpleDateFormat dateFormat = new SimpleDateFormat(datePattern, Locale.US);
			java.text.SimpleDateFormat normalDateFormat = new SimpleDateFormat(normalDatePattern);
